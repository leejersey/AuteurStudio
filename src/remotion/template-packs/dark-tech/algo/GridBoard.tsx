...
// 根据 state 返回样式
const getCellStyle = (state: string, colors: { primary: string; secondary: string; tertiary: string }) => {
  switch (state) {
    case "empty":
    ...
    >
      ...
      {grid.map((row, r) => (
        <div key={r} style={{ display: "flex", gap: cellGap }}>
          {row.map((cell, c) => {
            const style = getCellStyle(cell.state, theme.colors);
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: isHighlight ? `${theme.colors.primary}15` : style.bg,
                  border: `2px solid ${isHighlight ? theme.colors.primary : style.border}`,
                  display: "flex",
                  flexDirection: "column",
                  alignItems: "center",
                  justifyContent: "center",
                  boxShadow: isHighlight
                    ? `0 0 24px ${theme.colors.primary}40`
                    : "none",
                ...
                {cell.state !== "empty" && (
                  <span
                    style={{
                      ...
                      color: isHighlight ? theme.colors.primary : style.text,
                    }}
                  >
                    {cell.value}
                  </span>
